...
## 25. 新增毕业论文 Markdown 初稿并同步答疑记录

本轮开始围绕本科毕业论文正文落稿，目标不是只搭目录，而是形成一份可以继续扩写和排版的 `Markdown` 初稿，同时保证表述尽量贴近当前项目真实实现状态。
...
- 文件：
  - `毕业论文初稿.md`
- 主要内容：
  - 按“绪论、相关理论与技术基础、问题建模与算法设计、系统分析与设计、系统实现、系统测试与结果分析、总结与展望”的结构完成整篇初稿
  - 补充中英文摘要、关键词和参考文献列表
  - 将项目中的真实实现链路纳入正文，包括：
    - `LMAPF` 问题建模
    - 负载均衡 `A*` 初始规划
...
- 主要处理原则：
  - 尽量使用研究型、论文型语言，避免口语化和说明书式表达
  - 强调段落之间的承接关系，保证章节上下文连续
  - 对已有实现部分进行具体描述，对尚未补齐的量化实验保持克制表述
- 特别处理：
  - 第 `6` 章没有虚构尚未跑完的对比数据
  - 先写成“实验环境与参数设置、功能测试、阶段性结果分析”的可扩展版本，便于后续补表和补图
...
- 新增内容：
  - 为什么参考目录风格下不宜铺太多二级标题
  - 这次论文 `Markdown` 初稿按什么边界与原则来写

## 26. 读取文献综述定稿并回改论文初稿

本轮在已有 `Markdown` 初稿基础上，进一步读取了外部论文资料中的文献综述定稿，以补强当前初稿在综述性、学术衔接和文献口径上的不足。

### 26.1 参考材料来源

- 读取文件：
  - `/Users/eller/Downloads/同步空间/shool/毕业论文/定稿/薛文清_文献综述_2.docx`
- 本轮使用方式：
  - 提炼其“研究现状结构、方法演进逻辑、系统架构写法、参考文献组织方式”
  - 不直接照搬原文段落，改为与当前项目实现相结合后重新落笔
...
- 文件：
  - `毕业论文初稿.md`
- 主要修改：
  - 重写第 `1.2` 节“国内外研究现状”，按国内研究、国外研究、混合式方法发展趋势重新组织内容
  - 强化第 `2.1` 节对 `MAPF` 到 `LMAPF` 的理论过渡
  - 强化第 `2.2` 节对负载均衡 `A*` 的研究动机与方法定位
  - 强化第 `2.3` 节对多智能体强化学习、通信与注意力机制的说明
  - 调整第 `4.2` 节系统总体架构表述，使其更符合论文中的“算法引擎 + 后端服务 + 前端可视化”口径
  - 调整第 `5.1`、`5.3` 节部分技术描述，使其与当前代码结构和训练链路更加贴合
  - 扩充参考文献列表，补入中英文综述与代表性论文
...
- 文件：
  - `解疑.md`
- 新增内容：
  - 已确认后来读取并吸收 `薛文清_文献综述_2.docx` 的内容
  - 说明本轮修改吸收了哪些部分、具体回改了哪些章节

## 27. 新增毕业论文答辩 PPT 提示词文档

本轮围绕答辩材料准备，新增了一份可直接发送给其他 AI 的毕业论文答辩 PPT 生成提示词，并同步把相关说明写入答疑文档。

### 27.1 新增提示词文档

- 文件：
  - `DEFENSE_PPT_PROMPT_CN.md`
- 主要内容：
  - 基于当前项目真实背景整理答辩 PPT 生成提示词
  - 明确论文题目、研究问题、技术路线、技术栈和系统能力
  - 强调不要编造尚未完成的实验数据
  - 规定输出结构为约 `14` 页的答辩 PPT 文案
  - 强制预留：
    - 系统用例图
    - 系统功能模块图
    - 系统业务流程图
...
- 文件：
  - `解疑.md`
- 新增内容：
  - 如何根据当前项目写一段可发给其他 AI 的毕业论文答辩 PPT 提示词
  - 提示词中应包含哪些上下文信息
  - 为什么需要明确预留图和伪代码位置

## 28. 答辩 PPT 新增关键算法伪代码页

本轮围绕毕业设计答辩材料完善，在现有 PPT 文稿中正式补入“关键算法伪代码”页面，并同步记录相关答疑说明，确保答辩表述与项目技术路线一致。

### 28.1 补充 PPT 文稿中的伪代码页面

- 文件：
  - `MIDTERM_PPT_CONTENT_CN.md`
- 主要修改：
  - 在“核心算法与训练框架”页后新增一页：
    - `关键算法伪代码：PP+DR 融合协同规划流程`
  - 采用适合答辩展示的双栏结构：
    - 左侧为简洁伪代码
    - 右侧为 `PP`、`DR` 和整体优势的 `3` 点说明
  - 伪代码输入输出统一为：
    - 输入：`地图 M，智能体集合 A，目标集合 G，最大步数 T`
    - 输出：`多机轨迹 P，吞吐量 R，冲突统计 C`
  - 页面内容明确突出：
    - `LB-A*`
    - `PICC/PeCC`
    - `MARL`
    - “先规划、再精炼”的融合主线
  - 同步将后续页面页码整体顺延，保持整份 PPT 文稿编号连续。
...
- 文件：
  - `解疑.md`
- 新增内容：
  - 为什么答辩里更适合放 `PP+DR` 融合总流程伪代码，而不是只放单一模块
  - 为什么该页更利于和开题报告术语、流程图表达保持一致
  - 该页在答辩中的定位是“讲清技术路线”，而不是展开复杂数学细节

## 29. 按参考范文风格重写毕业论文摘要

本轮根据用户提供的本科论文摘要范文，对当前论文初稿中的中英文摘要进行了重写，目标是让摘要更符合毕业论文常见写法，而不是停留在项目说明或算法简介层面。
...
- 文件：
  - `毕业论文初稿.md`
- 主要修改：
  - 将中文摘要改写为“研究背景 -> 研究目的 -> 系统分析与算法设计 -> 技术实现 -> 阶段性效果”的结构
  - 弱化原先偏算法综述式的开头，增强“系统设计与实现”的论文表达
  - 在摘要中补入：
    - 需求分析
    - 任务持续到达与冲突处理问题
    - 系统功能链路与原型实现
...
- 文件：
  - `毕业论文初稿.md`
- 主要修改：
  - 使英文摘要与中文摘要在信息结构上保持一致
  - 对“系统设计与实现”“分层技术栈”“阶段性功能验证”等内容做对应翻译
...
- 文件：
  - `解疑.md`
- 新增内容：
  - 说明这次摘要是如何参考范文风格进行重写的
  - 明确本轮修改借鉴的是摘要结构，而不是直接照搬原文内容

## 30. 进一步简化英文摘要表述

本轮根据用户要求，对论文初稿中的英文摘要继续做了一次轻量收缩，目标是在不改变核心含义的前提下，使英文表达更简洁，并较前一版减少约 `20` 个词。
...
- 主要修改：
  - 压缩部分过长句式和重复修饰
  - 将少量偏重的表达改为更直接的学术英语
  - 保持研究背景、两阶段算法、技术栈和阶段性验证四类核心信息不变
...
- 新增内容：
  - 为什么英文摘要又做了一次简化
  - 说明本轮修改重点是“收短表达”，而不是改变摘要内容

## 31. 合并摘要前两段

本轮根据用户反馈，继续压缩摘要版式，将前两段合并为一个更紧凑的段落，以降低摘要的视觉长度。
...
- 新增内容：
  - 说明为什么继续把摘要前两段合并
  - 说明本轮调整重点是压缩版式而不是删减信息

## 32. 在论文初稿中补入系统图题与表格占位

本轮根据用户提供的系统相关图片，对论文初稿中的正文结构进行了补强，在不直接插入图片的前提下，为后续正式排版预留了图题、引用语句和表格说明。
...
- 文件：
  - `毕业论文初稿.md`
- 主要修改：
  - 在第 `4.1` 节补入 `图 4.1 系统用例图` 的正文引用与图题占位
  - 在第 `4.3` 节补入 `图 4.2 系统功能模块图` 的正文引用与图题占位
  - 在第 `4.3` 节补入 `图 4.3 系统概念 ER 图` 的正文引用与图题占位
  - 在第 `4.4` 节补入 `图 4.4 系统业务流程图` 的正文引用与图题占位
...
- 主要修改：
  - 新增 `表 4.1 系统主要功能模块说明`
  - 用表格对任务管理与调度、无人机监控与控制、运行可视化与统计、用户信息管理四类模块进行归纳说明
...
- 文件：
  - `解疑.md`
- 新增内容：
  - 说明这批系统图片分别适合放在论文的哪个章节和哪个小节
  - 说明当前 `md` 中采用的是“图题占位 + 正文引用”的处理方式

## 33. 补充 `PP` 与 `DR` 的术语解释

本轮围绕答辩术语说明，补充了 `PP` 与 `DR` 的标准含义，便于在 PPT、答辩讲稿和论文表述中保持一致。
