...
- 主要修改：
  - 将 `patrol` 类任务默认映射到 `campus` 模板，语义从一次性仓储巡检调整为园区巡逻；
  - 新增 `continuous_patrol` 支持：当任务为持续巡逻时，到达当前回放末帧不会结束，而是回到巡逻起点继续下一轮；
  - 增加循环偏移量统计（步数偏移、累计完成任务数偏移），保证长期巡逻时指标持续累加；
  - 新增任务控制动作 `set_speed`，允许运行中动态修改 `tick_ms`；
  - 修复持续巡逻回放元数据中的 `map_name` 读取错误。

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 在飞手“任务执行”页将固定速度按钮改为可填写的速度输入框；
  - 新增“设置速度”按钮，可直接提交自定义 `tick_ms`；
  - 页面同步展示当前飞行节拍（`ms/步`），便于答辩演示时直观看到速度变化。
  - 在管理员和飞手的“回放与告警”区域新增 `回放速度(ms/帧)` 输入框；
  - 支持独立设置历史回放播放节拍，播放中修改后立即按新速度继续播放。

- 文件：`解疑.md`
- 主要修改：
  - 新增“园区巡逻任务和普通一次性任务有什么不同”问答；
  - 新增“飞手界面为什么增加了速度调节按钮”问答。

## 17. 申请人 -> 管理员 -> 飞手 任务链路打通与权限补全

- 文件：`web_demo/task_runtime.py`
- 主要修改：
  - 为任务列表、任务详情、告警、反馈、历史回放增加基于角色的访问控制；
  - 权限规则调整为：
    - 管理员：可查看和操作全部任务；
    - 申请人：仅可查看自己提交的任务申请；
    - 飞手：仅可查看和操作分配给自己的任务；
  - 为任务控制动作增加角色限制，飞手只能操作自己被分配的任务，申请人不能直接控制执行任务。

- 文件：`backend_django/views.py`
- 主要修改：
  - 为任务详情、告警、反馈、回放接口补充权限错误透传；
  - 当前接口在无权限访问时会返回明确的 `403` 错误，而不是模糊的空数据。

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 在申请人任务页新增“申请详情”卡片；
  - 可直接查看当前申请的审核状态、任务类别、申请地点、指派飞手和审核意见，形成完整可展示链路。

- 文件：`解疑.md`
- 主要修改：
  - 新增“申请人、管理员、飞手之间的任务链路现在是否真的打通了”问答；
  - 记录这次链路验证结果与权限边界。
...
- 文件：`web_demo/task_runtime.py`
- 主要修改：
  - 将管理员、申请人、飞手 10 个账号的默认密码统一为 `1`；
  - 后端重启后会重新同步 `auth_credentials` 中的密码哈希。

- 文件：`解疑.md`
- 主要修改：
  - 更新登录说明，明确当前默认账号密码均已改为 `1`；
  - 补充申请人角色的默认登录信息。

## 19. 审核表单草稿保留修复（地图切换导致飞手被清空）

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 为待审核任务新增 `reviewFormTaskId` 草稿绑定；
  - 同一条申请在未提交审核前，不再因轮询刷新重复从后端回填表单；
  - 仅在切换到另一条待审核任务时，才重新同步飞手、地图、时间等审核字段。

- 修复效果：
  - 管理员在审核页切换地图、修改时间、选择飞手时，表单内容不会被自动刷新覆盖；
  - 解决了“点击地图后飞手选择被取消”的问题。

- 文件：`解疑.md`
- 主要修改：
  - 新增“为什么管理员在审核时刚选好的飞手会突然被清空”问答，记录问题根因与修复方式。
...
## 89. 补充文献综述 3.2 技术介绍的算法版改写建议

- 文件：`解疑.md`
- 主要修改：
  - 新增“文献综述 3.2 技术介绍如果不写前后端，改写成算法介绍应该怎么写”问答；
  - 按当前项目真实实现，整理出适合替换进 Word 的 `3.2 技术介绍` 正文；
  - 改写内容围绕以下算法与方法展开：
    - `LMAPF` 问题建模
    - `SMAPO` 两阶段协同优化框架
    - `LB-A*` 初始路径规划
...
## 91. 2D 地图新增无人机彩色虚线路径层

- 文件：`web_frontend/src/modules/shared/renderer.js`
- 主要修改：
  - 在共享 2D 渲染器中新增“路径层”；
  - 每架无人机会根据当前所在位置、目标点和地图障碍，计算一条可通行路径；
  - 路径使用与无人机本体一致的颜色绘制；
  - 路径样式改为虚线，并增加随时间变化的 `lineDashOffset`，形成轻微闪动与流动效果；
  - 路径绘制顺序放在障碍物之后、目标圈和无人机图标之前，避免遮挡关键元素。
...
## 92. 修复历史回放页未真正生效的 2D 路径动画

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 新增历史回放页的轻量画布刷新定时器；
  - 当管理员位于“已完成任务”或飞手位于“回放与告警”页签时，会周期性重绘历史回放 canvas；
  - 飞手切换到“回放与告警”页签时，改为自动调用 `loadReplay()`，不再依赖手动点“加载历史回放”后才出现回放内容；
  - 关闭详情页、切出历史页签或离开页面时，会正确停止这类定时器，避免无效刷新。

- 修复结果：
  - 历史回放中的 2D 路径不再只是静态首帧；
  - 即使停留在同一帧，虚线也会继续闪动；
  - 飞手侧历史回放页签进入后能自动加载对应任务回放。
...
## 94. 新增可复用的 Anthropic 风格 UI 设计规范文档

- 文件：`web_frontend/UI_STYLE_GUIDE_CN.md`
- 主要修改：
  - 新增全站 UI 风格指南文档，作为后续页面重构的统一基准；
  - 明确系统重构方向为“暖白编辑风 + 深色局部功能舞台”，而非继续沿用当前大面积深色控制台风格；
  - 固化品牌气质、禁止项、颜色系统、字号梯度、圆角、阴影、间距和最大内容宽度等设计令牌；
  - 补充导航栏、Hero、按钮、表单、信息卡片、列表项、状态标签、2D/3D 视图容器等组件规范；
  - 为登录页、申请人页、管理员页、飞手页、运营大屏分别定义页面模板与布局原则；
  - 补充“现有页面重构映射”“实施顺序”“开发约束”“验收标准”，保证后续实现不再重复做风格决策。

- 设计目标：
  - 借鉴 Anthropic 的视觉语言与信息编排；
  - 保留系统原有任务链路和 2D/3D 核心能力；
  - 将页面从后台控制台风格，重构为更适合毕业设计展示的研究型产品风格。

## 95. 第一轮入口层 UI 重构：登录页与顶层导航切换为暖白编辑风

- 文件：`web_frontend/src/App.vue`
- 主要修改：
  - 重构登录页结构，从单卡片登录框改为“左侧编辑化介绍 + 右侧登录卡片”的双栏入口布局；
  - 新增登录页导语、三段能力说明和更清晰的身份切换入口；
  - 顶层已登录页面改为新的标题区文案系统，根据申请人、管理员、飞手三个角色动态显示不同页面命题与说明；
  - 调整导航标签文案，使其更接近展示型系统语气，例如“审核与调度”“运营观测”。

- 文件：`web_frontend/src/styles.css`
- 主要修改：
  - 新增暖白编辑风的顶层视觉变量，包括页面背景、卡片底色、边界色、正文色和柔和阴影；
  - 将 `body`、`app-shell`、`login-shell`、`topbar`、`mode-tabs` 等入口层容器切换为暖白编辑风；
  - 重写登录页、账号卡片、角色切换 segmented control、退出按钮和顶层 tab 的样式；
  - 增加轻量淡入与位移动画，并补充桌面与窄屏下的基础适配。
...
## 97. 第三轮任务详情层收束：飞手执行、反馈、回放界面进一步减密

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 飞手任务详情区新增 `TASK DETAIL`、`MISSION CONTROL`、`FLIGHT SPEED`、`FIELD NOTES`、`TASK REPLAY` 等分段标识；
  - 将原本直接堆叠的执行按钮和速度设置拆成两个更清晰的动作卡片；
  - 将飞手详情页 tab 文案压缩为“执行 / 反馈 / 回放”，减少噪音；
  - 反馈页和回放页增加更明确的小标题与简短说明，使执行、反馈、回放三层关系更清楚。

- 文件：`web_frontend/src/styles.css`
- 主要修改：
  - 新增 `executor-action-grid`、`executor-action-card`、`executor-toolbar` 等样式；
  - 将飞手任务详情中的动作区整理为更安静的双卡布局；
  - 调整任务状态标签样式，使任务卡片中的状态更接近轻量标签而不是强调色块；
  - 为中等宽度屏幕补充详情动作区的单列适配，避免操作块挤压。

- 设计效果：
  - 任务详情不再是“按钮 + 输入框 + 回放条”的线性堆积；
  - 执行动作、速度控制、现场反馈和历史回放被分成更清楚的层次；
  - 在不改业务逻辑的前提下，让飞手执行页更接近展示型产品，而不是后台调试面板。

- 验证结果：
  - `npm --prefix web_frontend run build` 通过；
  - 使用 Playwright 复查登录后的主页面与申请人首页，页面结构正常渲染；
  - 新增动作分区样式未造成任务详情层构建错误。

## 98. 补充页面拆分策略说明，明确后续 UI 精简方向

- 文件：`解疑.md`
- 主要修改：
  - 新增“当前系统页面太多字和数据时，应该怎么合理拆分”的问答；
  - 明确页面不应再按技术模块堆叠，而应按“申请 -> 审核 -> 执行 -> 回放”的任务链路拆分；
  - 给出申请人、管理员、飞手、运营侧的推荐页面拆分方式；
  - 固化“列表页只负责浏览与筛选、详情页只负责单任务操作”的分层原则。

- 设计结论：
  - 一个页面应只承担一个主要目的；
  - 管理员应拆成“审核申请 / 执行调度 / 任务归档”；
  - 飞手应拆成“任务列表 + 单任务详情（执行 / 反馈 / 回放）”；
  - 运营大屏应单独承担展示，不再混入审批或复杂表单。

## 99. 第四轮页面拆分落地：管理员与申请人改成更轻的工作区结构

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 将管理员 `审核 / 执行 / 归档` 三个页面进一步拆成“左侧列表 + 右侧单任务详情”的轻工作区；
  - `审核` 页改成待审核申请列表 + 当前申请审批面板，减少审批页同时展示多组参数；
  - `执行` 页改成执行中任务列表 + 当前任务联合运行详情，移除重复的状态说明块；
  - `归档` 页改成历史任务列表 + 当前任务复盘面板，减少回放区前的重复数据堆叠；
  - 新增申请人页签：`发起申请 / 我的申请`，把申请提交与申请追踪拆开；
  - 将“我的申请”改成左侧申请列表、右侧申请详情的形式，避免一屏同时出现表单和所有历史状态。

- 文件：`web_frontend/src/styles.css`
- 主要修改：
  - 新增 `admin-subheadline`、`admin-focus-header`、`admin-focus-badge` 等更轻的结构样式；
  - 为管理员审批表单和申请人表单新增更规整的双列表单布局；
  - 增加 `requester-create-grid`、`requester-history-grid`，让申请人工作区按任务流拆开；
  - 调整响应式规则，确保新拆分结构在常见桌面尺寸下仍保持单屏清晰。

- 文件：`解疑.md`
- 主要修改：
  - 补充“最近这一轮 UI 拆分实际落地了什么”的说明；
  - 明确记录管理员三页和申请人两页已从概念拆分进入实际界面结构。

- 设计效果：
  - 管理员不再在同一屏里同时读列表、调参数、看回放和看反馈；
  - 申请人不再在同一屏同时提交任务和阅读全部申请细节；
  - 每个工作区现在更接近“先选任务，再处理当前任务”的真实操作节奏。

- 验证结果：
  - `npm --prefix web_frontend run build` 通过；
  - 模板切换、申请人页签切换、管理员三页结构重组后未引入构建错误。

## 100. 第五轮减字与减密：内嵌联合运行视图和状态提示继续收束

- 文件：`web_frontend/src/modules/operations/OperationsModeView.vue`
- 主要修改：
  - 将嵌入式联合运行区标题改成更轻的 `MISSION STAGE / 实时任务舞台`；
  - 把原先独立的任务 ID 文本改成更紧凑的元信息标签；
  - 在嵌入模式下只保留 `在线无人机数` 和 `累计完成任务` 两个核心指标；
  - 在嵌入模式下隐藏无人机表格和实时告警，让管理员执行页、飞手执行页只保留真正用于展示的 2D/3D 画面。

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 将审批页、申请页、飞手执行页中的默认说明型状态文案移除，改成只有操作后才出现反馈；
  - 将飞手页“当前飞行节拍”并入速度卡片内部，减少执行页中部多余的一行文字；
  - 保持所有业务功能不变，只压缩说明文字和重复状态提示。

- 文件：`web_frontend/src/styles.css`
- 主要修改：
  - 新增嵌入式运行区的轻量化样式 `ops-main-panel-embedded` 和 `ops-inline-meta`；
  - 让管理员执行页中的嵌入任务舞台更像单独的展示块，而不是重复一层面板。

- 设计效果：
  - 管理员执行页和飞手执行页中的联合运行区明显更安静；
  - 页面上只保留当前决策所需的数据，减少“默认说明句”和重复统计项；
  - 同样的业务链路现在更适合演示，不容易让老师在一屏里被大量文字打断。

- 验证结果：
  - `npm --prefix web_frontend run build` 通过；
  - 使用浏览器实际查看飞手任务详情页，嵌入运行区已只保留任务舞台、少量元信息和核心指标。

## 101. 第六轮入口与运营页收口：去掉冗余页签，运营观测改成焦点任务卡片

- 文件：`web_frontend/src/App.vue`
- 主要修改：
  - 当当前角色只有一个可见页面时，不再显示顶层页签导航；
  - 管理员和申请人登录后不会再看到“只有一个 tab 还占一整排”的冗余结构。

- 文件：`web_frontend/src/modules/dashboard/OpsDashboardView.vue`
- 主要修改：
  - 将运营观测页左侧从大表格改成“少量焦点任务卡片”；
  - 保留任务总数、运行中、已完成、平均吞吐量 4 个总体指标；
  - 右侧焦点区只保留快照画面和 4 个核心指标，不再重复任务 ID 和状态卡；
  - 运营页更接近展示型工作台，而不是后台列表页。

- 文件：`web_frontend/src/modules/taskcenter/TaskCenterView.vue`
- 主要修改：
  - 申请人 `发起申请` 与 `我的申请` 继续减字，新增更轻的计数条替代解释文字；
  - 飞手任务列表页加入轻量任务计数条，去掉默认刷新提示句；
  - 列表页只保留筛选和任务本身，不再让状态说明抢占视觉焦点。

- 文件：`web_frontend/src/styles.css`
- 主要修改：
  - 新增运营页卡片堆叠样式；
  - 保持整体暖白编辑风，同时让运营页视觉上更接近“观察台”。

- 设计效果：
  - 登录后的顶层结构更简洁，不会因为单页面角色出现冗余导航；
  - 运营观测页从“表格导向”变成“焦点任务导向”；
  - 申请人和飞手列表页进一步减少说明文字，任务本身更突出。
