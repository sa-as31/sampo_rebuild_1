...
def main():
    ...
    assert planner_agent.obstacles.shape == (4, 26, 26), planner_agent.obstacles.shape
    assert len(positions[0]) == 3, positions[0]
    assert env.grid.config.height_levels == 4, env.grid.config.height_levels
