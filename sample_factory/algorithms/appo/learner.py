...
def build_rnn_inputs_sep(x, dones_cpu, rnn_states, T: int):
    ...
    x = x.chunk(2, dim=1)
    x_seq_0 = PackedSequence(x[0].index_select(0, select_inds), batch_sizes, sorted_indices)
    x_seq_1 = PackedSequence(x[1].index_select(0, select_inds), batch_sizes, sorted_indices)
    ...
    return [x_seq_0, x_seq_1], rnn_states, inverted_select_inds
...
def build_core_out_from_seq_sep(x_seq: PackedSequence, inverted_select_inds):

    return [x_seq[0].data.index_select(0, inverted_select_inds), x_seq[1].data.index_select(0, inverted_select_inds)]
...
class LearnerWorker:
    def __init__(
        ...
        self.task_queue = MpQueue()     # the share task_queue
        self.report_queue = report_queue 

        self.initialized_event = MultiprocessingEvent()   # torch.multiprocessing.Event 类--多进程事件中的同步
        ...
        self.policy_worker_queues = policy_worker_queues  # policy_workers
        ...
        self.num_discarded_rollouts = 0      # the number of discards rollouts
        ...
        self.save_evaluation_model = False
    ...
    def deferred_initialization(self):
        ...
        self.training_thread = Thread(target=self._train_loop) if self.train_in_background else None  # asign training_thread 
        self.train_thread_initialized = threading.Event()    # 用于线程之间的通知与等待
    ...
    def _broadcast_model_weights(self):
        ...
        # Send model_state to all policy_worker use this learner_worker
    
    def _get_oth(self, ids_oth, adv):
        oth_adv = np.zeros_like(ids_oth)
        for i in range(ids_oth.shape[0]):
            for j in range(ids_oth.shape[1]):
                oth_adv[i,j] = adv[i, ids_oth[i,j] + j]
        return oth_adv.transpose(1,0,2)

    def _get_oth_id(self, ids_oth):
        for i in range(ids_oth.shape[0]):
            for j in range(ids_oth.shape[1]):
                ids_oth[i,j] += j
        return ids_oth.transpose(1,0,2)
    ...
    def _calculate_gae(self, buffer):
        ...
        for i in range(len(values_arr)):
            last_value, last_reward = values_arr[i][-1], rewards[i, -1]   # last time's value reward
    ...
    def _macro_batch_size(self, batch_size):
        return self.cfg.num_batches_per_iteration * batch_size   # 1 * 1024
    ...
    def _process_rollouts(self, rollouts, timing):
        ...
        rollouts_in_macro_batch = self._macro_batch_size(batch_size) // self.cfg.rollout  # 1024 / 8 = 128
        ...
        policy_version = self.train_step
        for r in rollouts:
            mask = r.t['policy_id'] == self.policy_id   # 查看 rollout 对应的 actor policy_id 是否与当前 policy_id 相同
            if np.any(mask):
                rollout_newest_version = r.t['policy_version'][mask].max().item() # 当前最新版的 policy_id
            else:
                log.error(
    ...
    def _save(self):
        ...
        if self.cfg.evaluation_saves_per_run:
            while len(self.evaluation_save_splits) and self.evaluation_save_splits[0] <= self.env_steps:
                self.evaluation_save_splits.pop(0)

                log.info('Saving evaluation model at step %d', self.env_steps)
                evaluation_dir = ensure_dir_exists(join(checkpoint_dir, 'evaluation'))
                evaluation_path = join(evaluation_dir, f'{checkpoint_name}.evaluation')
                shutil.copy(filepath, evaluation_path)
                self.save_evaluation_model = False
    ...
    def calc_G_sum(self, G_output, mask):
        G_masked = G_output * mask
        G_sum = G_masked.sum(dim = 1)
        mask_sum = mask.sum(dim = 1)
        check_zero = mask_sum < 1e-3
        mask_sum[check_zero] = 1e-3
        return (G_sum / mask_sum).squeeze(-1)
    
    def calc_adv_sum(self, adv_oth, mask):
        adv_masked = adv_oth * mask
        adv_s = adv_masked.sum(dim = 1)

        mask_sum = mask.sum(dim = 1)
        # if mask is long or bool Tensor it will ignore this:
        check_zero = mask_sum < 1e-3
        mask_sum[check_zero] = 1e-3
        return adv_s / mask_sum.squeeze(-1)

    def _G_loss(self, G_output, target, valids):
        G_loss = (G_output - target).pow(2)
        G_loss = G_loss[valids]
        G_loss = G_loss.mean()
        G_loss *= self.cfg.G_loss_coeff
        ...
        for epoch in range(self.cfg.ppo_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with torch.no_grad():
                    ...
                    clip_ratio_high = 1.0 + final_clip_ratio 
                ...
                with timing.add_time('bptt_initial'):
                    if self.cfg.use_rnn:
                        if self.cfg.actor_critic_share_weights:
                            ...
                            head_output_seq, rnn_states, inverted_select_inds = build_rnn_inputs_sep(
                                head_outputs, mb.dones_cpu, mb.rnn_states, recurrence,
                            )
                    else:
                ...
                with timing.add_time('bptt'):
                    if self.cfg.use_rnn:
                        ...
                        if self.cfg.actor_critic_share_weights:
                            ...
                            core_outputs = build_core_out_from_seq_sep(core_output_seq, inverted_select_inds)
                    else:
                ...
                with torch.no_grad():  
                    ...
                    adv_mean = adv.mean()
                    adv_std = adv.std()
                    adv = (adv - adv_mean) / max(1e-7, adv_std)  
                ...
                with timing.add_time('losses'):
                    ...
                    kl_loss = self.kl_loss_func(self.actor_critic.action_space, mb.action_logits, action_distribution,
                                                valids)
    ...
    def _accumulated_too_much_experience(self, rollouts):
        ...
        rollouts_per_minibatch = self.cfg.batch_size / self.cfg.rollout  # 1024 / 8 = 128
    ...
    def set_evaluation_save_flag(self):
        log.error('set_evaluation_save_flag')
        self.save_evaluation_model = True
