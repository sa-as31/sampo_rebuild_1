...
def mlp_encoder(cfg, in_features, out_features):
    Layers = []
    for _ in range(cfg.G_mlp_encoder_layer):
        Layers.extend([ 
...
class Attention_cob(EncoderBase):
    def __init__(self, cfg, timing):
        super().__init__(cfg, timing)
        ...
        self.sqer_len = torch.tensor(self.head_dim).sqrt()
    ...
    def forward(self, x, ids, mask, relative_xy):
        ...
        q = self.Q(self_embedding).view(batch_size, -1, self.num_heads, self.head_dim)
        k = self.K(oth_embedding).view(batch_size, -1, self.num_heads, self.head_dim)
        v = self.V(oth_embedding).view(batch_size, -1, self.num_heads, self.head_dim)
        ...
        x = attention * v
        x = torch.sum(x, dim = -2).view(batch_size, -1) + self_embedding
...
def create_action_encoder(cfg, timing):
    return ActionEncoder(cfg, timing)


class ActionEncoder(EncoderBase):
    def __init__(self, cfg, timing):
        super().__init__(cfg, timing)
    ...
    def forward(self, one_hot_actions):
        x = self.action_encoder(one_hot_actions)
        x = self.forward_fc_blocks(x)
        return x
...
def make_G_decoder(cfg):
    Layers = []
    for _ in range(cfg.G_decoder_layer):
        Layers.extend([nn.Linear(cfg.hidden_size, cfg.hidden_size),
                       nonlinearity(cfg)])
    Layers.extend([nn.Linear(cfg.hidden_size, 1)])
    return nn.Sequential(*Layers)
